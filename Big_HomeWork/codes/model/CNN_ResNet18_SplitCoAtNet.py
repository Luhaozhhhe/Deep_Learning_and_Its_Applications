...
class SplitAttention(nn.Module):
    ...
    def __init__(self, in_channels, channels, kernel_size=3, stride=1, padding=1, 
                 groups=2, radix=2, reduction_factor=4, norm_layer=nn.BatchNorm2d):
        ...
        # 使用分组卷积实现特征分割
        self.conv = nn.Conv2d(
            in_channels, channels*radix, kernel_size=kernel_size,
            stride=stride, padding=padding, groups=groups*radix, bias=False)
    ...
    def forward(self, x):
        ...
        # 分割特征图
        splited = torch.split(x, self.channels, dim=1)
        gap = sum(splited)
        ...
        # 计算注意力权重
        atten = self.fc2(gap).view(batch, self.radix, self.channels, 1, 1)
        atten = F.softmax(atten, dim=1).view(batch, -1, 1, 1)
        
        # 应用注意力权重
        atten = torch.split(atten, self.channels, dim=1)
...
# 结合ResNet18、CoAtNet和Split Attention的基本块
class ResNetSplitCoAtBlock(nn.Module):
    def __init__(self, inchannel, outchannel, stride=1, image_size=(8, 8), 
                 heads=4, dim_head=32, dropout=0., radix=2, groups=1):
        super(ResNetSplitCoAtBlock, self).__init__()
        
        # 第一个卷积层使用Split Attention
        self.split_attn = SplitAttention(
            inchannel, outchannel, kernel_size=3, stride=stride, padding=1,
            groups=groups, radix=radix, reduction_factor=4
        )
        self.bn1 = nn.BatchNorm2d(outchannel)
        self.relu = nn.ReLU(inplace=True)
        
        # 使用CoAtNet的注意力机制作为第二个模块
        h, w = image_size
        # 计算第一个卷积后的特征图尺寸
        if stride == 2:
            h, w = h // 2, w // 2
...
class ResNet18_SplitCoAtNet(nn.Module):
    def __init__(self, num_classes=100, dropout=0.1, radix=2, groups=1):
        super(ResNet18_SplitCoAtNet, self).__init__()
        ...
        # 创建四个阶段，每个阶段2个块
        self.layer1 = self.make_layer(64, 2, stride=1, image_size=(32, 32), heads=4, radix=radix, groups=groups)
        self.layer2 = self.make_layer(128, 2, stride=2, image_size=(32, 32), heads=4, radix=radix, groups=groups)
        self.layer3 = self.make_layer(256, 2, stride=2, image_size=(16, 16), heads=8, radix=radix, groups=groups)
        self.layer4 = self.make_layer(512, 2, stride=2, image_size=(8, 8), heads=8, radix=radix, groups=groups)
    ...
    def make_layer(self, channels, num_blocks, stride, image_size, heads, radix, groups):
        ...
        for i, stride in enumerate(strides):
            ...
            layers.append(ResNetSplitCoAtBlock(
                self.inchannel, 
                channels, 
                stride=stride,
...
def ResNet18SplitCoAtNet_Tiny(num_classes=100):
    """Tiny版本的ResNet18-SplitCoAtNet混合模型"""
    return ResNet18_SplitCoAtNet(
...
def ResNet18SplitCoAtNet_Small(num_classes=100):
    """Small版本的ResNet18-SplitCoAtNet混合模型，增加radix"""
    return ResNet18_SplitCoAtNet(
        num_classes=num_classes,
        dropout=0.2,
        radix=4,
...
def ResNet18SplitCoAtNet_Base(num_classes=100):
    """Base版本的ResNet18-SplitCoAtNet混合模型，进一步增加radix和groups"""
    return ResNet18_SplitCoAtNet(
        num_classes=num_classes,
        dropout=0.3,
        radix=4,
