...
class SEBlock(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d(1)  # 输出大小为 (B, C, 1, 1)
...
class ResNetHorBlock(nn.Module):
    def __init__(self, inchannel, outchannel, stride=1, drop_path=0., layer_scale_init_value=1e-6, order=3):
        ...
        self.se = SEBlock(outchannel)  # 添加SE模块
    ...
    def forward(self, x):
        ...
        gnconv_out = self.se(gnconv_out)  # 插入SE模块
        if self.gamma is not None:
...
def ResNet18HorNetSE_Tiny(num_classes=100):
    """Tiny版本的ResNet18-HorNet混合模型"""
    return ResNet18_HorNet_SE(
        num_classes=num_classes,
        drop_path_rate=0.1,
        layer_scale_init_value=1e-6
    )

def ResNet18HorNetSE_Small(num_classes=100):
...
def ResNet18HorNetSE_Base(num_classes=100):
